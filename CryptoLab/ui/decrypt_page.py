...
class DecryptPage(ctk.CTkFrame):
    """Page for decrypting Fernet encrypted text."""
    ...
    def _build_page(self):
        ...
        description = ctk.CTkLabel(
            self,
            text="الصق النص المشفَّر أدناه واستخدم مفتاح Fernet لاستعادة الرسالة الأصلية.",
            anchor="e",
            wraplength=820,
            justify="right",
            text_color="#d0d8e7",
        ...
        decrypt_button.grid(row=0, column=0, padx=(0, 10))
        ...
        note = ctk.CTkLabel(
            self,
            text="تأكد من تحميل المفتاح الصحيح قبل فك تشفير النص.",
            anchor="e",
            wraplength=820,
            justify="right",
            text_color="#99a3b5",
    ...
    def decrypt_text(self):
        token = self.input_text.get("1.0", "end").strip()
        try:
            if not token:
                raise ValueError("يرجى لصق النص المشفَّر لفك التشفير.")
            if self.app.current_key is None:
                raise ValueError("حمل أو أنشئ مفتاحاً قبل فك التشفير.")

            plaintext = decrypt_message(self.app.current_key, token)
            self.output_text.delete("1.0", "end")
            self.output_text.insert("1.0", plaintext)
            self.app.display_status("تم فك التشفير بنجاح.", success=True)
        except Exception as error:
            messagebox.showerror("خطأ في فك التشفير", str(error))
            self.app.display_status("فشل فك التشفير. تحقق من المفتاح والنص.", success=False)
    ...
    def copy_result(self):
        ...
        if not result:
            messagebox.showwarning("تنبيه النسخ", "لا يوجد نص مفكوك للنسخ.")
            return

        self.clipboard_clear()
        self.clipboard_append(result)
        messagebox.showinfo("تم النسخ", "تم نسخ النص المفكوك إلى الحافظة.")
